...
for Time in range(0,N-1): #这里就不需要在循环的末尾写time自加1了
    ...
    for i in range(NUMB):
        ...
        action= Qnetwork.getAction(actionNum=NUMA*3,stateInput=state_old)
        Qnetwork.action_all[i, 0] = action % NUMRB
        Qnetwork.action_all[i, 0] = int(np.floor(action / NUMRB))
        reward = Env.get_reward(stateinput=state_old,actionall=Qnetwork.action_all,idx=i)
        state_new = Env.get_state()

        loss = Qnetwork.getLoss(currentState=state_old,nextState=state_new,action=action,reward=reward)
        R_total += reward
